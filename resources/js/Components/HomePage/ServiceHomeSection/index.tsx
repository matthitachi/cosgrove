...
export default function ({ serviceItems, cmsSection }: ServiceList) {
    React.useEffect(() => {
        ...
        <section className={styles.offerHomeSection}>
            <Container>
                <h4 data-aos="fade-right" className={`subHeader mb-25`}>
                    {(cmsSection?.data?.label as string) ?? 'Our Services'}
                </h4>
                <h2 data-aos="fade-right" className={`headerDark  mb-65`}>
                    {(cmsSection?.data?.heading as string) ?? 'What we offer our clients'}
                </h2>
                <Row>
                    <Col data-aos="fade-right" md={6} className={`pe-3 pe-md-5`}>
                        <p
                            ...
                            {(cmsSection?.data?.subheading as string) ?? "Step into a world where your needs shape our services. Offering bespoke consultations, innovative design & automation, meticulous construction, and strategic project management, we make your journey seamless. Plus, unlock smart investment avenues for unparalleled ROI. With Cosgrove, it's more than a home—it's a lifetime investment."}
                        </p>
                        <ul className={styles.serviceList}>
                            {
                                (() => {
                                    const cmsItems = cmsSection?.data?.items as Array<{ img: string; title: string; description: string }> | undefined;
                                    if (cmsItems?.length) {
                                        return cmsItems.map((item, index) => (
                                            <ServiceItem key={index} img={item.img} title={item.title} desc={item.description} />
                                        ));
                                    }
                                    return serviceItems.map((item, index) => <ServiceItem key={index} {...item} />);
                                })()
                            }
                        </ul>
                    </Col>
                    <Col
                        ...
                        style={{
                            background:
                                'url("/assets/images/services/CGVLF2.jpg")',
                        }}
                    ></Col>
                </Row>
            </Container>
